...
namespace bat.opt.Bake.unit
{
    ...
    public class BAT_BakeGroup_Deep:BAT_BakeGroup
    {
        ...
        public void CollectMeshFilter(MeshFilter meshFilter, Material material)
        {
        ...
        public void Bake(Transform _root, BAT_ShaderConfig shaderConfig)
        {
            ...
            while (beginID < m_BakeUnits.Count)
            {
                ...
                if (currentID >= 0 && currentID < m_BakeUnits.Count)
                {
                    ...
                    mesh = m_meshes[unitI.m_meshID];
                    vertexCountI = mesh.vertexCount;
                }
                ...
                if (needBake)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        var BakeUnitI = m_BakeUnits[beginID + i]as BAT_BakeUnit_Deep;
                        combineInsts[i].mesh = m_meshes[BakeUnitI.m_meshID];
                        combineInsts[i].transform = BakeUnitI.m_meshFilter.transform.localToWorldMatrix;
                    ...
                    meshFilterRoot.sharedMesh = BakedMesh;
