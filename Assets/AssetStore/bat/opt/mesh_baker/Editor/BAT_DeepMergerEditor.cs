...
namespace bat.opt.Bake
{
    ...
    public class BAT_DeepBakerEditor : Editor
    {
        ...
        {
            var meshRenderers = m_deepBaker.GetComponentsInChildren<MeshRenderer>();
            ...
            {
                var mt = meshRenderer.sharedMaterial;
                if (mt != null && !mtList.Contains(mt))
                {
                    ...
                    {
                        var list = BAT_GfxUtil.ListTextures(shader);
    ...
    public class Texture2DItem
    {
        ...
        private static GUILayoutOption LayNoExpWidth = GUILayout.ExpandWidth(false);
        public void Fix()
        {
            if (!HasTrouble())
            {
