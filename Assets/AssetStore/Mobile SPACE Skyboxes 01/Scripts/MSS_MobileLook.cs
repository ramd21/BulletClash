...
public class MSS_MobileLook : MonoBehaviour {
    ...
    void Update() {
		...
		transform.RotateAround(transform.right, Input.acceleration.y * sensitivity * Time.deltaTime);
		transform.RotateAround(transform.up, Input.acceleration.x * sensitivity * Time.deltaTime);		
